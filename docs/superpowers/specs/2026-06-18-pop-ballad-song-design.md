# Design: a pop ballad in song mode (two slots)
...
Add a buzzer melody arrangement of a well-known pop ballad to **song mode**:
...
The ballad sits in a relative minor→major key pair (verse in a minor mode, the
...
1–2 so the signature rising chorus hook reads clearly on the buzzer.

This is a **melodic reduction** — recognizable contour within the monophonic,
white-key, diatonic envelope — not a faithful reproduction of any recording.
...
- `SONG_BALLAD[]` — verse + pre-chorus + chorus. Est. ~60–80 pairs
  (~120–160 B PROGMEM).
- `SONG_BALLAD_HOOK[]` — the chorus phrase only. Est. ~16–28 pairs.
...
- `firmware/child_buzzer/config.h`
  ...
  - Add `SONG_BALLAD_HOOK[]` (slot 3) and `SONG_BALLAD[]` (slot 4).
  - Update `SONGS[]`, `SONG_LEN[]`, and the per-key comments to the new roster.
- `README.md` — update the songs list (key 4 = pop-ballad chorus hook, key 5 =
  full pop ballad).
...
Acceptance: both keys play a recognizable rendition of the ballad, the full
version moves through verse → pre-chorus → chorus, tempo responds to the knob,
and the other five songs still play. Record the final compiled flash size.
