...
// Key 3: pop-ballad chorus hook. Monophonic white-key reduction in C major.
// The signature rise: the held syllable leaps up to C6 (SN(0,2)), then the
// figure shifts up a step so the next peak is D6 (SN(1,2)). Resolves on the
// tonic C. See the song-mode design notes under docs/superpowers/specs/.
const uint8_t SONG_BALLAD_HOOK[] PROGMEM = {
  SN(4,1),2, SN(4,1),2, SN(0,2),6,  SN(5,1),2, SN(5,1),2, SN(1,2),6,        // rising hook pair
...
// Key 4: full pop-ballad arrangement — verse -> pre-chorus -> chorus, played
// once. Verse anchored in A natural minor, chorus in its relative C major; both
// sit on white keys so the whole tune needs no accidentals (the format has
// none). Verse low (band 1), chorus lifts to bands 1-2.
const uint8_t SONG_BALLAD[] PROGMEM = {
  // verse
...
const uint8_t* const SONGS[7] PROGMEM = {
  SONG_TWINKLE, SONG_MARY, SONG_ODE, SONG_BALLAD_HOOK, SONG_BALLAD, SONG_RIFF, SONG_STOMP,
};
const uint8_t SONG_LEN[7] = {  // number of {note, duration} pairs per song
  ...
  sizeof(SONG_BALLAD_HOOK) / 2, sizeof(SONG_BALLAD) / 2, sizeof(SONG_RIFF) / 2,
  sizeof(SONG_STOMP) / 2,
};
